$("#creer_page").on("submit", function(c){
    c.preventDefault();
    var data = new FormData();
    $.each($('form#creer_page :input'), function(i, fileds){
       data.append($(fileds).attr('name'), $(fileds).val());
    });

    var champs = ["titre","contenu"];
    ...
    $.ajax({
        ...
        success: function(jqXHR){
            if(jqXHR[0] == true){
                swal({
                    title: "Succès",
                    text: "Votre page a bien été crée",
                    type: "success",
                    timer: 4000,
