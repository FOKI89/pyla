...
{   
    ...
    public function form_connect(){
        ...
        if($this->input->post("mdp") != $mdp){
            $return[1] = "Les identifiants saisis ne sont pas reconnus\nVeuillez les ressaisir";
            die(json_encode($return));
        }
        if($user_tuple->statut > 2){
            ...
            die(json_encode($return));
        }elseif($user_tuple->statut < 0 || empty($user_tuple->statut)){
            show_error("Id statut","error_db");
            return false;
        }
